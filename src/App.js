...
export default class App extends Component {
  ...
  render() {
    let dic = { 北美: 85, 西北欧: 67, 南美: 55, 东南亚: 24, 地中海: 43, 日本: 42, 波斯湾: 35, 非洲: 15, 澳洲: 15, 台湾: 10, 香港: 6, 韩国: 8, 其他: 3, }
    let round = 5
    sealineData.forEach(element => {
      let times = Math.pow(dic[element.areaN], 0.2) || 0
      for (let i = 0; i < times; i++) {
        let data = element.points;
        let newData = data.map(e => { return [e[0] + (Math.random() - 0.5) * round, e[1] + (Math.random() - 0.5) * round] })
